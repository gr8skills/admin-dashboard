    ...
    <div class="card-body">
        <form action="{{ route("admin.mainMenu.store") }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <input type="text" id="name" name="name" class="form-control" placeholder="{{ old('name', isset($menu) ? $menu->name : '') }}" required>
                @if($errors->has('name'))
                    <p class="help-block">
                        {{ $errors->first('name') }}
                    </p>
            ...
            <div class="form-group {{ $errors->has('link') ? 'has-error' : '' }}">
                ...
                <input type="text" id="link" name="link" class="form-control" placeholder="{{ old('link', isset($menu) ? $menu->link : '') }}" >
                @if($errors->has('link'))
                    <p class="help-block">
                        {{ $errors->first('link') }}
                    </p>
                @endif
                <p class="helper-block">
                    {{ trans('cruds.menu.fields.link_helper') }}
                </p>
            </div>
            <div class="form-group {{ $errors->has('submenu') ? 'has-error' : '' }}">
                <label for="submenu">{{ trans('cruds.menu.fields.submenu') }}*</label>
                <textarea id="submenu" name="submenu" rows="4" class="form-control" placeholder="{{ old('submenu', isset($menu) ? $menu->submenu : '') }}" ></textarea>
            ...
            <div class="form-group {{ $errors->has('alias') ? 'has-error' : '' }}">
                ...
                <input type="text" id="alias" name="alias" class="form-control" placeholder="{{ old('alias', isset($menu) ? $menu->alias: '') }}">
                @if($errors->has('alias'))
                    <p class="help-block">
                        {{ $errors->first('alias') }}
                    </p>
